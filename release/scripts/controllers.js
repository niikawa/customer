 ...
 */
var approachCtrl = angular.module('approachCtrl', [
    'ApproachServices',
    'ScenarioServices'
  ]);
approachCtrl.controller('ApproachCtrl', [
  '$scope',
  '$routeParams',
  'Shared',
  'Utility',
  ...
  function ($rootScope, $scope, $sce, Upload, Shared, Bug, Modal, Utility) {
    ...
    $scope.vote = function (index) {
      Bug.resource.vote({ id: $scope.bugList[index].id }).$promise.then(function (response) {
        getInfo();  //            $scope.bugList[index].comments = response.data;
      });
    };
    function saveComment() {
      if (0 === $scope.modalParam.newComment.length)
        return;
      var params = {
...
mainCtrl.controller('DashbordCtrl', [
  '$scope',
  'Shared',
  'Scenario',
  'Utility',
  ...
  function ($scope, $location, Auth, Location) {
    ...
    $scope.submit = function () {
      ...
      Auth.login($scope.data).then(function (response) {
        ...
        Location.home();  //Auth.setLoginStatus(response.data.user_id);
      }, function () {
        $scope.$emit('requestEnd');
      });
    };
...
queryListCtrl.controller('QueryListCtrl', [
  '$scope',
  'Shared',
  'Query',
  'Segment',
  ...
  function ($scope, Shared, Query, Segment, Modal, Location, Utility) {
    ...
    $scope.showSegment = function (index) {
      if (void 0 === $scope.queryList[index].id)
        return;
    ...
    $scope.deleteItem = function (index) {
      if (void 0 === $scope.queryList[index])
        return;
...
queryCtrl.controller('QueryCtrl', [
  ...
  function ($scope, $routeParams, Shared, Query, Location, Utility) {
          ...
          angular.forEach($scope.tableList[tableName].column, function (columnData) {
            if (columnInfo.column === columnData.physicalname) {
              $scope.selectColumns.push({
                table: {
                  logicalname: $scope.tableList[tableName].logicalname,
                  physicalname: $scope.tableList[tableName].physicalname
                },
                column: columnData,
                selectedCondition: {
                  name: '',
                  value: columnInfo.conditionType,
                  symbol: ''
                },
    ...
    $scope.release = function (pIndex, cIndex) {
      ...
      if (1 === $scope.showConditions[pIndex].length)
        $scope.showConditions[pIndex].isJoin = false;
...
scenarioCtrl.controller('ScenarioCtrl', [
  ...
  function ($scope, $routeParams, Shared, Utility, Scenario) {
    ...
    $scope.remove = function (index) {
      if (void 0 === index || isNaN(parseInt(index, 10)))
        return false;
...
]);var scenarioControlCtrl = angular.module('scenarioControlCtrl', [
    'ScenarioServices',
    'SegmentServices'
  ]);
scenarioControlCtrl.controller('ScenarioControlCtrl', [
  '$scope',
  '$routeParams',
  'Modal',
  'Shared',
  ...
  function ($scope, $routeParams, Modal, Shared, Utility, Location, Scenario) {
     ...
     */
    function setInitializeScope() {
      $scope.isBaseCollapse = false;
      //基本設定Collapse制御
      $scope.isOriginCollapse = false;
      //個別設定Collapse制御
      $scope.isShowExtraction = false;
      //抽出条件表示制御
      ...
      $scope.asyncValidators = {
        scenario_name: {
          same: function (modelValue, viewValue) {
            var val = modelValue || viewValue;
            if (void 0 === val || val.length === 0)
              return true;
            return Scenario.validators.isSameName({
              id: id,
              scenario_name: val
            });
          }
    ...
    function getInitializeData() {
      ...
      Scenario.resource.initializeData(params).$promise.then(function (response) {
        ...
        if (1 === pageProp.type) {
          if (isEdit)
            editScheduleInitialize(response);
        } else if (2 === pageProp.type) {
        ...
        angular.forEach(conditionItems, function (items) {
          angular.forEach($scope.columnList, function (info) {
            if (items.physicalname == info.physicalname) {
              ...
              restoration.push(info);  //return false;
            }
          });
        });
    ...
    $scope.save = function () {
      ...
      if (1 === pageProp.type) {
        ...
        //ベータ版のための制御
        if (0 === selectConditionList.length)
          return false;
        doc = {
          actionId: actionId,
...
segmentCtrl.controller('SegmentCtrl', [
  '$scope',
  'Shared',
  'Segment',
  'Utility',
  function ($scope, Shared, Segment, Utility) {
    ...
    $scope.remove = function (index) {
      ...
      Segment.resource.remove({
        id: target.segment_id,
        segment_document_id: target.segment_document_id
      }).$promise.then(function (response) {
        Utility.info('\u524a\u9664\u3057\u307e\u3057\u305f');
...
]);var segmentControlCtrl = angular.module('segmentControlCtrl', [
    'SegmentServices',
    'QueryServices'
  ]);
segmentControlCtrl.controller('SegmentControlCtrl', [
  '$scope',
  '$routeParams',
  'Modal',
  'Shared',
 ...
 * Controller of the workspaceApp
 */
var userControlCtrl = angular.module('userControlCtrl', [
    'UesrServices',
    'RoleServices'
  ]);
userControlCtrl.controller('UserControlCtrl', [
  '$scope',
  '$routeParams',
  'User',
  'Role',
