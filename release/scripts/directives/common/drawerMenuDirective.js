...
myApp.directive('drawerMenuDirective', function () {
  return {
    ...
    template: '<nav class="hi-icon-effect-8 drawer-menu">' + '<a ng-repeat="item in itemList" style="{{item.style}}" class="drawer-menu-item" ng-href={{item.link}}>' + '<i ng-if="item.fa" class="fa {{item.fa}} fa-3x"></i>' + '<span ng-if="item.s">{{item.s}}</span>' + '</a>' + '</nav>',
