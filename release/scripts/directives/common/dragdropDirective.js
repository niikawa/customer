...
myApp.directive('dragItemDirective', [
  'DDShared',
  function (DDShared) {
    return {
      restrict: 'AE',
      ...
      link: function (scope, element, attrs, ctrl) {
        ...
        element.on('dragover', function (event) {
          ...
          if (void 0 !== event.target.dataset.index) {
            ...
            var wholeheight = Math.max.apply(null, [
      ...
      link: function (scope, element, attrs, ctrl) {
        ...
        element.on('drop', function (event) {
          ...
          if (angular.isArray(ctrl.$modelValue)) {
            //画面別
            if (ctrl.$modelValue[0].hasOwnProperty('table') && ctrl.$modelValue[0].hasOwnProperty('column')) {
              if (ctrl.$modelValue[0].table.physicalname == pushItem[0].table.physicalname && ctrl.$modelValue[0].column.physicalname == pushItem[0].column.physicalname)
                return false;
            }
            isModelArray = true;
          }
